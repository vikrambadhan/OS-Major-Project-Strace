...
extern int sys_setoptions(void);
...
static int (*syscalls[])(void) = {
    ...
    [SYS_trace] sys_trace,   [SYS_setoptions] sys_setoptions,[SYS_setname] sys_setname,
};
...
void syscall(void) {
  ...
  // so print it here only
   ...
   if(num == SYS_exit && is_traced && proc->option == 0) 
   {
  ...
  if (num > 0 && num < NELEM(syscalls) && syscalls[num]) 
  {
    ...
    if (is_traced) {
        ...
        //    cprintf("%d",proc->option);
           ret = proc->tf->eax;
           filter = strncmp(proc->flag,syscall_names[num],strlen(proc->flag));
        //    cprintf("%d",filter);
        //    cprintf("%d",(proc->option ==1 && filter));
           if((proc->option == 1 && filter == 0) || proc->option==0 || (proc->option==2  && ret>=0) || (proc->option==3  && ret == -1) || (proc->option==4))
           {
...
//     int num, i;
//     num = proc->tf->eax;
//     int is_traced = proc->traced;
//     char procname[16];
//     i=0;
//     int matched = 1;
//     // struct file *f;
//     while(syscall_names[num][i]!='\0')
//     {
//         if(syscall_names[num][i]!=proc->flag[i])
//         {
//             matched = 0;
//             break;
//         }
//         i+=1;
//     }
//     //cprintf("matched value is: %d",matched);
//     // copy process name
//     for(i=0; proc->name[i] != 0; i++) 
//     {
//         procname[i] = proc->name[i];
...
//     procname[i] = proc->name[i];
//     int val = proc->tf->eax;
//     if((num == SYS_exit) && (is_traced || proc->option==4)) 
//     {
//         val = proc->tf->eax;
//         if((proc->option ==1 && matched) ||proc->option==0 || (proc->option==2  && val>=0) || (proc->option==3  && val == -1) || (proc->option==4))
//         {
...
//         }
//     }
//     if ((num > 0 && num < NELEM(syscalls) && syscalls[num]) || proc->option==4) 
//     {
//         proc->tf->eax = syscalls[num]();
//         if (is_traced || proc->option==4) 
//         {
//             success = proc->tf->eax;
//             if((proc->option ==1 && matched) || proc->option==0 || (proc->option==2  && val>=0) || (proc->option==3  && val ==-1) || (proc->option==4))
//             {
